use crate::util::{feq, fgeq, fgt, Coordinate};
use std::fmt;
...
impl Ray {
    ...
    pub(crate) fn bisector(&self, rhs: &Self, origin: Coordinate, orient: bool) -> Self {
    ...
    ///
    /// ```
    pub fn intersect(&self, rhs: &Self) -> Coordinate {
        let op = self.angle.outer_product(&rhs.angle);
        if feq(op, 0.) {
            if self.is_contain(&rhs.origin) {
    ...
    pub fn rotate_by(&self, angle: f64) -> Self {
        let nx = self
            .angle
            .0
            .mul_add(f64::cos(angle), -(self.angle.1 * f64::sin(angle)));
        let ny = self
            .angle
            .0
            .mul_add(f64::sin(angle), self.angle.1 * f64::cos(angle));
        Self {
            origin: self.origin,
            angle: (nx, ny).into(),
        }
